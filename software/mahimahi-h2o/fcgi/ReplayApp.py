...
class ReplayApp:
    ...
    def _init_custom_scheduler(self):
        ...
        env = os.environ
        if env['H2O_CUSTOM_SCHEDULER'] == "1" and self.push_strategy_file != "noop":
            self.has_custom_strategy = True
            self.custom_strategy_host_trigger = env['H2O_CUSTOM_TRIGGER_HOST']
            self.custom_strategy_path_trigger = env['H2O_CUSTOM_TRIGGER_PATH']
            self.custom_strategy_offset_html = int(env['H2O_CUSTOM_OFFSET'])

            custom_strategy_html_content_file = os.path.join(os.path.dirname(self.push_strategy_file), env['H2O_CUSTOM_CONTENT_FILE'])

            print "Custom Replay Scheduler active for "+env['H2O_CUSTOM_TRIGGER_HOST']
            ...
            critical_css_file = os.path.join(os.path.dirname(self.push_strategy_file), env['H2O_CRITICAL_CSS_FILE'])
