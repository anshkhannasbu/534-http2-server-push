...
for identifier, url in all_domains:
    ...
    print identifier
    print url
    cmd = 'mitmdump -n -q -s mitm2html.py --set htmlurl \"{}\" -r {} | gzip > {}'.format(url,capture_file,output_file)
    print(cmd)
    pv = subprocess.Popen(
        'mitmdump -n -q -s mitm2html.py --set htmlurl={} -r {} | gzip > {}'.format(url,capture_file,output_file),
        shell=True)
    pv.communicate()
